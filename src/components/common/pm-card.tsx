  ...
  return (
    ...
    >
      ...
      {hasPhoto ? (
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900">
          <div className="absolute inset-0 flex items-center justify-center">
            <span className="text-7xl font-extralight tracking-widest text-white/20">{initials}</span>
          </div>
          {/* Subtle geometric decoration */}
          <div className="absolute top-8 right-8 h-32 w-32 rounded-full border border-white/[0.06]" />
          <div className="absolute top-12 right-12 h-24 w-24 rounded-full border border-white/[0.04]" />
          <div className="absolute bottom-32 left-8 h-16 w-16 rounded-full border border-white/[0.05]" />
        </div>
      )}
      ...
      <div className="absolute top-4 left-4 right-4 z-10 flex items-center justify-between">
        ...
        {pm.projects.length > 0 && (
          <div className="flex items-center gap-1.5 rounded-full bg-black/40 px-3 py-1.5 backdrop-blur-md">
            ...
            <span className="text-[11px] font-medium text-white/90">{pm.projects.length}</span>
          </div>
        )}
      </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-5">
        ...
        {/* Title */}
        {pm.title && (
          <p className="mt-1 text-sm text-white/50">{pm.title}</p>
        )}
